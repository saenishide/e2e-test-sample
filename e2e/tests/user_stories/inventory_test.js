...
Scenario("店舗スタッフは、デフォルトの注文可能数を変更できる。ユーザーは、デフォルトの注文可能数まで商品を注文できる。", ({ I, utils }) => {
    // 事前準備：　商品データを作成する
    ...
    // 商品を追加する
    // 商品名は、タイムスタンプなどからユニークなものを設定する。例えば、「牛ハラミ弁当-テスト-20230416120600」などのようにする
    // 商品説明と価格はそれぞれ「テスト用の商品です」と「500」にする
    I.click("商品を追加する");
    ...
    session("user", () => {
        // 商品を10個カートに入れる
        I.amOnPage("/items");
        I.fillField(locate("input").after(locate("label").withText("カートに入れる数量").inside(itemContainer)), "10");
...
Scenario.only("店舗スタッフは、デフォルトの注文可能数を変更できる。ユーザーは、デフォルトの注文可能数を超えて注文するとエラーになる。", ({ I, utils }) => {
    const itemName = `牛ハラミ弁当-テスト-${utils.now.format("YYYYMMDDHHmmss")}`;
    const itemContainer = locate("tr").withText(itemName);
